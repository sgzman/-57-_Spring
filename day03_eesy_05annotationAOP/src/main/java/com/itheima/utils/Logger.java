...
public class Logger {
     ...
     *
     * 分析：
     *      通过对比动态代理中的环绕通知代码，发现动态代理中的环绕通知有明确的的切入点方法调用，而我们的代码中没有.
     *
     * 解决：
     *      Spring框架为我们提供了一个接口，ProceedingJoinPoint。该接口有一个方法proceed()，此方法就相当于明确调用切入点方法
     *      该接口可以作为环绕通知的方法参数，再程序执行时 spring框架会为我们提供该接口的实现类供我们使用
     *
     * spring中的环绕通知：
    ...
    public Object aroundPrintLog(ProceedingJoinPoint pjp){
        Object rtVale=null;
        try{
            Object[] args=pjp.getArgs();//得到方法运行所需要的的参数
            System.out.println("前置通知:Logger类中的aroundPrintLog方法开始记录日志了。");
            rtVale=pjp.proceed(args);//明确调用业务层方法，也叫切入点方法
